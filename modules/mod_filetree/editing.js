/*
* Allows editing in file tree module
...
//show descriptions for editing buttons
$(".actionButton").hover(
	function(){
		$("#actionDescription").html($(this).attr("description"));
		},
	function(){
...
function addAction(actionP,type){
	var parts = window.command.split(",");
	//add file name if necessary/otherwise continue with link
	if(type=="file"){
		if(parts[0]=="delete"||parts[0]=="rename"||parts[0]=="move"){
			window.command += "," + actionP;
		}
		else{
			return true;
		}
	}
	//if not file name, add command
	if(type==null)
		window.command = actionP;
	...
	if(parts.length>0){
		...
		if(parts[0]=="delete" && parts.length == 2)
			return runAction();
		if(parts[0]=="rename" && parts.length == 2){
			$.prompt('Enter a new name: <input id="newName" type="text">',{ submit: function(){window.command += "," + $("#newName").val();runAction();} });
			return false;
			}
...
function runAction(){
	$("#commandField").val(window.command).parent().submit();
	return false;
}
/*
 * jQuery Impromptu
