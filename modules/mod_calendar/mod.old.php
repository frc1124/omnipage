...
if(!mysql_fetch_array(mysql_query("SELECT * FROM `calendarMentors` WHERE userId = '".$user->data["user_id"]."' AND eventId = '".mysql_real_escape_string($_GET["mentors"])."'")))
				...
				*/
				...
				while($row = mysql_fetch_array($oneDay)){
					...
					if($user->data["user_id"]!=ANONYMOUS)
						$mentors = "<p><a href=\"/o/calendar?mentors=".$row['id']."\">Mentors</a></p>";

					$event .= "<span><a href=\"javascript:void(0);\" id=\"".$row['id']."\" class=\"eventLink\">
".(empty($minute)?"":"<img height=\"12\" src=\"/omni/skins/classic/images/minutes-small.png\">").$row['name']."</a>
								...
								$mentors
								$minute
								".($pagePermissions?"<a href=\"http://uberbots.org/forums/posting.php?mode=post&amp;f=58&amp;calendarEvent=".$row["id"]."&amp;subject=".urlencode(date("D n-j-y",$row['startTime'])." Minutes")."\">+ Add Meeting Minutes</a>":"")."</div></span>";
...
<br>
<div id=\"startDate\">
<label for=\"startDate\" style=\"width:150px;display:inline-block;\">Start Date:</label><input type=\"text\" name=\"startMonth\" id=\"startMonth\" size=\"1\" maxlength=\"2\" value=\"MM\"> <input type=\"text\" name=\"startDay\" id=\"startDay\" size=\"1\" maxlength=\"2\" value=\"DD\"> <input type=\"text\" name=\"startYear\" id=\"startYear\" size=\"2\" maxlength=\"4\" value=\"YYYY\"></div> <div id=\"endDate\"><label for=\"endDate\" style=\"width:150px; display:inline-block;\">End Date: </label><input type=\"text\" name=\"endMonth\" id=\"endMonth\" size=\"1\" maxlength=\"2\" value=\"MM\"> <input type=\"text\" name=\"endDay\" id=\"endDay\" size=\"1\" maxlength=\"2\" value=\"DD\"> <input type=\"text\" name=\"endYear\" id=\"endYear\" size=\"2\" maxlength=\"4\" value=\"YYYY\"></div>
<br>
